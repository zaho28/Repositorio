...
router.post('/inventario', upload.single('imagen_producto'), async (req, res) => {
    ...
    try {
        ...
        if (es_nuevo_producto === 'true') {
            ...
            const consul_actualiza_stock = ` UPDATE producto SET stock_actual = stock_actual + ?, ultima_actualiz = ? WHERE id_producto = ? `;
            await new Promise((resolve, reject) => {
                db.query(consul_actualiza_stock, [cantidad, ultima_actualiz, producto_id], (err, result) => { 
                    if (err) return reject(err); 
                    resolve(result); 
                });
            });
        ...
        await new Promise((resolve, reject) => {
            db.query(insertar_movimiento, movementValues, (err, result) => { 
                if (err) return reject(err); 
    ...
    // =======================================================================
    // POST (CREATE) REGISTRO DE SALIDA (Venta/Disminución de Stock)
    // =======================================================================
    ...
    console.log("Registrando salida de inventario:");
    console.log({ id_producto, cantidad, id_usuario });

    // Validación 
    if (!id_producto || !cantidad || !id_usuario) {
        return res.status(400).json({ 
            error: "Faltan campos obligatorios (id_producto, cantidad, id_usuario)" 
        });
    }

    const cantidadSalida = parseInt(cantidad);
    if (cantidadSalida <= 0) {
        return res.status(400).json({ 
    ...
    try {
        ...
        const consul_verifica_stock = `
            SELECT stock_actual, nom_producto 
            FROM producto 
            WHERE id_producto = ? AND estado = 1
        `;
        
        const resultados_stock = await new Promise((resolve, reject) => {
            db.query(consul_verifica_stock, [id_producto], (err, results) => {
                if (err) return reject(err);
                resolve(results);
            });
        });

        if (resultados_stock.length === 0) {
            throw new Error("Producto no encontrado o inactivo");
        }

        const stockActual = resultados_stock[0].stock_actual;
        const nombreProducto = resultados_stock[0].nom_producto;

        if (stockActual < cantidadSalida) {
            throw new Error(`Stock insuficiente. Disponible: ${stockActual}, Solicitado: ${cantidadSalida}`);
        }
        ...
        await new Promise((resolve, reject) => {
            db.query(consul_actualiza_stock, [cantidadSalida, ultima_actualiz, id_producto], (err, result) => {
                if (err) return reject(err);
        ...
        // Registrar movimiento de SALIDA en la tabla movimiento
        const insertar_movimiento = `
            INSERT INTO movimiento (Cantidad_m, fecha_m, observaciones, id_m, id_producto, id_usuario) 
            VALUES (?, ?, ?, ?, ?, ?)
        `;
        ...
        const movementValues = [
            ...
            'M-S',  //ID de movimiento para SALIDA
            id_producto,
            id_usuario
        ];
        ...
        await new Promise((resolve, reject) => {
            db.query(insertar_movimiento, movementValues, (err, result) => {
                if (err) return reject(err);
                console.log("Movimiento de salida registrado:", result.insertId);
                resolve(result);
            });
        });
        ...
        console.log("Salida de inventario completada exitosamente");

        res.status(201).json({ 
            message: "Salida registrada con éxito", 
            id_producto,
            cantidad: cantidadSalida,
            nuevo_stock: stockActual - cantidadSalida
        });

    } catch (error) {
        console.error("Error en transacción de salida:", error);
...
// =======================================================================
// POST (CREATE) REGISTRO DE VENTA COMPLETA CARRITO
// =======================================================================
router.post('/venta-carrito', async (req, res) => {
    const { 
        items,  // Array de productos: [{id_producto, cantidad}, ...]
        id_usuario,
        metodo_pago 
    } = req.body;

    console.log("Procesando venta de carrito completo");

    if (!items || items.length === 0 || !id_usuario) {
        return res.status(400).json({ 
            error: "Se requiere items y id_usuario" 
        });
    }
    ...
    try {
        ...
        for (const item of items) {
            ...
            try {
                ...
                await new Promise((resolve, reject) => {
                    db.query(consul_actualiza_stock, [cantidad, ultima_actualiz, id_producto], (err, result) => {
                        if (err) return reject(err);
                        resolve(result);
                    });
                });
                ...
                await new Promise((resolve, reject) => {
                    db.query(insertar_movimiento, [
                        cantidad,
                        ultima_actualiz,
                        `Venta - Método: ${metodo_pago || 'No especificado'}`,
                        'M-S',
                        id_producto,
                        id_usuario
                    ], (err, result) => {
        ...
        // Si hay errores críticos, revertir cambios
        if (errores.length > 0 && resultados.length === 0) {
            throw new Error("No se pudo procesar ningún producto del carrito");
        }
        ...
        console.log("Venta de carrito procesada");

        res.status(201).json({ 
            message: "Venta procesada",
            exitosos: resultados,
            errores: errores.length > 0 ? errores : null,
            total_procesado: resultados.length
        });

    } catch (error) {
        console.error("Error en venta de carrito:", error);
    ...
    router.delete('/:id', (req, res) => {
        ...
        const consul_eliminar = `
            UPDATE producto 
            SET estado = 0, ultima_actualiz = NOW()
            WHERE id_producto = ? AND estado = 1
        `;

        db.query(consul_eliminar, [id], (err, result) => {
            if (err) {
