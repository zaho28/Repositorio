...
CREATE TABLE producto (
  id_producto INT PRIMARY KEY AUTO_INCREMENT NOT NULL, -- Sintaxis limpia para INT PK AUTO_INCREMENT
...
-- Modificar el ENUM para agregar "Sin clasificar"
ALTER TABLE clasificacion 
MODIFY nombre_clas ENUM(
    "Sin clasificar",
    "En oferta", 
    "Mas vendidos", 
    "Nuevos", 
    "Ultimas unidades"
) NOT NULL DEFAULT "Sin clasificar";

-- Insertar las clasificaciones
INSERT INTO clasificacion (nombre_clas) VALUES 
    ('Sin clasificar'),
    ('En oferta'),
    ('Mas vendidos'),
    ('Nuevos'),
    ('Ultimas unidades')
ON DUPLICATE KEY UPDATE nombre_clas = nombre_clas;

-- Hacer que id_clasificacion sea obligatorio (NOT NULL)
ALTER TABLE producto 
MODIFY COLUMN id_clasificacion INT NOT NULL DEFAULT 1;

-- Si se tiene productos con NULL, actualizarlos
UPDATE producto 
SET id_clasificacion = 1 
WHERE id_clasificacion IS NULL;

-- Ver todos los registros con sus IDs
SELECT * FROM clasificacion ORDER BY id_clasificacion;

-- Eliminar SOLO los duplicados (IDs mayores a 5)
-- Primero actualiza los productos que usan estos IDs
UPDATE producto 
SET id_clasificacion = CASE 
    WHEN id_clasificacion = 6 THEN 1  -- Si apunta al duplicado 6, cambiarlo a 1
    WHEN id_clasificacion = 7 THEN 2  -- duplicado 7 -> 2
    WHEN id_clasificacion = 8 THEN 3  -- duplicado 8 -> 3
    WHEN id_clasificacion = 9 THEN 4  -- duplicado 9 -> 4
    WHEN id_clasificacion = 10 THEN 5 -- duplicado 10 -> 5
    ELSE id_clasificacion
END
WHERE id_clasificacion > 5;

-- Ahora sí puedes eliminar los duplicados
DELETE FROM clasificacion WHERE id_clasificacion > 5;
...
VALUES
('Adm-01', 'Valentina', '', 'Ruiz', 'Castro', 'valruiz@gmail.com', 3123456789, '$2b$10$ZpYbdvjxoxOFc9H1WE.9v.sSNaEvHqRHlThGiMvDAYy/StkwtxK6a', '$2b$10$jQpj1gMJypBF/d2zQEGz2OdfRTlvBzhMUj2nrkHxV./I/tNMqymke', '1', 'CC', ''),

-- CATEGORIAS
INSERT INTO categoria (nombre_c, descripcion) VALUES
('Sabanas', 'Sabanas con encaje en todos los tamaños'),
('Cubrelechos', 'Cubrelechos con diseños'),
('Amigurumis', 'Muñecos tejidos'),
('Llaveros', 'Llaveros tejidos a mano');
...
-- TIPO DE PEDIDO
-- CORREGIDO: El ENUM de la tabla es ENUM("P-P", "P-E") y los valores son ENUM("Personalizado", "Estandar"). Se insertan los valores de la clave (P-P, P-E)
...
ALTER TABLE usuario 
ADD COLUMN codigo_visible VARCHAR(20) NULL 
COMMENT 'Código en texto plano solo para visualización administrativa';

UPDATE usuario 
SET codigo_visible = '12345' 
