...
class PersonalizacionSabanasScreen extends StatefulWidget {
  ...
  @override
  State<PersonalizacionSabanasScreen> createState() => _PersonalizacionSabanasScreenState();
}

class _PersonalizacionSabanasScreenState extends State<PersonalizacionSabanasScreen> {
        ...
        setState(() {
          if (results[0].statusCode == 200) {
            _colores = (jsonDecode(results[0].body) as List)
                .map((e) => ColorMaterialModel.fromJson(e)).toList();
          }
          if (results[1].statusCode == 200) {
            _disenos = (jsonDecode(results[1].body) as List)
                .map((e) => DisenoMaterialModel.fromJson(e)).toList();
          }
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'materiales': [
            {'id_material': _telaSeleccionada!.idMaterial, 'cantidad': _metrosTotales.toInt()},
          ],
        }),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Personalizar Sabana',
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(entry.value['dimensiones'],
                            style: const TextStyle(fontSize: 10, color: Color(0xFF5a3d54))),
                      ],
                    ),
                  ),
                ],
    ...
    return _contenedor(
      ...
      child: Column(
        children: _telas.map((tela) {
          final seleccionada = _telaSeleccionada?.idMaterial == tela.idMaterial;
          return GestureDetector(
            onTap: () {
              setState(() {
  ...
  Widget _seccionColor() {
    if (_cargandoColores) {
      return const Padding(
        ...
        child: Center(child: CircularProgressIndicator(color: Color(0xFFc45a77))),
      );
    }
    ...
    return _contenedor(
      ...
      child: Wrap(
        ...
        children: _colores.map((color) {
          final seleccionado = _colorSeleccionado?.idColor == color.idColor;
    ...
    return _contenedor(
      ...
      child: Column(
        children: _disenos.map((diseno) {
          final seleccionado = _disenoSeleccionado?.idDiseno == diseno.idDiseno;
          return GestureDetector(
            onTap: () => setState(() => _disenoSeleccionado = seleccionado ? null : diseno),
            child: Container(
              margin: const EdgeInsets.only(bottom: 8),
              padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 12),
              decoration: BoxDecoration(
              ...
              child: Row(
                children: [
                  if (seleccionado) const Icon(Icons.check, color: Color(0xFFc45a77), size: 16),
        ...
        children: [
          Row(
            children: [
              Checkbox(
                ...
                onChanged: (v) => setState(() => _incluirSobresabana = v ?? false),
                activeColor: const Color(0xFFc45a77),
              ),
              const Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Incluir sobresabana',
                      style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF5a3d54))),
                  Text('+2 metros de tela adicionales',
                      style: TextStyle(fontSize: 12, color: Color(0xFF5a3d54))),
                ],
              ),
            ],
          ),
          ...
          ...['Sin fundas', 'Una funda', 'Dos fundas'].map((opcion) {
            final descripcion = opcion == 'Sin fundas'
                ? 'No incluir'
                : opcion == 'Una funda' ? '+1 metro de tela' : '+2 metros de tela';
                ...
                child: Row(
                  children: [
                    Radio<String>(
                      ...
                      onChanged: (v) => setState(() => _fundasAlmohada = v!),
                      activeColor: const Color(0xFFc45a77),
                    ),
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
        ...
        children: [
          if (_telaSeleccionada != null) ...[
            Row(
              ...
              children: [
                const Text('Tela:', style: TextStyle(color: Color(0xFF5a3d54))),
            ...
            if (_colorSeleccionado != null)
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Color:', style: TextStyle(color: Color(0xFF5a3d54))),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Metros:', style: TextStyle(color: Color(0xFF5a3d54))),
