...
function ModalRegistrar({ onClose, onGuardado }) {
    const [form, setForm] = useState({
        ...
        codigo_trabajador: generarCodigo(),
        id_rol_usuario: '3',
    });
    ...
    const handleSubmit = async () => {
        setError('');
        if (!form.nom_1 || !form.ape_1 || !form.correo || !form.contrasena) {
            ...
            <div style={s.modal}>
                ...
                <div style={s.modalCampo}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <input style={{ ...s.modalInputReadonly, flex: 1 }} value={form.codigo_trabajador} readOnly />
                        <button
                            onClick={() => setForm(p => ({ ...p, codigo_trabajador: generarCodigo() }))}
...
export default function Usuarios() {
    ...
    const handleCambiarEstado = async (usuario) => {
        ...
        try {
            await apiPatch(`/usuarios/${usuario.id_usuario}`, { estado: nuevoEstado });
                    ...
                    <div style={{ background: '#fff', borderRadius: '14px', border: `1px solid ${C.borde}`, overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
                        ...
                        <div style={s.tablaWrap}>
                            <table style={s.tabla}>
                                <thead>
                                    <tr>
                                        {['Usuario', 'Contacto', 'Rol', 'Estado', 'Registro', 'Acciones'].map(h => (
                                ...
                                <tbody>
                                    {visibles.length === 0 ? (
                                        ...
                                        <tr key={u.id_usuario} style={{ background: i % 2 === 0 ? '#fff' : C.rosaLight }}>
                                            ...
                                            <td style={{ ...s.td, fontSize: '13px', color: C.gris }}>
